...
interface TracerProjectPickerProps {
  item?: LibraryItem; // Optional now if used for generic selection
  onClose: () => void;
  onSelect?: (project: TracerProject) => void; // New Prop for Selection Mode
}

const TracerProjectPicker: React.FC<TracerProjectPickerProps> = ({ item, onClose, onSelect }) => {
  ...
  const handleAction = async (project: TracerProject) => {
    if (onSelect) {
      onSelect(project);
      onClose();
      return;
          ...
          <div className="p-5 md:p-8 border-b border-gray-100 flex items-center justify-between shrink-0 bg-gray-50/50">
             <div className="flex items-center gap-3 md:gap-4">
                ...
                <div>
                   ...
                   <p className="text-[8px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest">{onSelect ? 'Select Target Project' : 'Select available research below'}</p>
                </div>
             </div>
